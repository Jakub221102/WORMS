...
GR::SampleDynamicObject::SampleDynamicObject(b2World& world, const float& time, std::vector<std::pair<float, float>> vertices, std::string texture_path)
...
void GR::SampleDynamicObject::putVelocity(float x, float y) {
	box2dModel->putVelocity({ x, y });
}

void GR::SampleDynamicObject::putVelocity(const sf::Vector2f& vec) {
	putVelocity(vec.x, vec.y);
}

void GR::SampleDynamicObject::update() {
...
void GR::SampleDynamicObject::setPosition(float x, float y, float angle) {
	box2dModel->setNewPosition({ x, y }, angle * 180.0f / PI);
}

void GR::SampleDynamicObject::setPosition(const sf::Vector2f& position, float angle) {
	setPosition(position.x, position.y, angle);
}

void GR::SampleDynamicObject::setRotationSpeed(float speed) {
	box2dModel->setRotationSpeed(speed);
}

void GR::SampleDynamicObject::translate(float x, float y) {
	box2dModel->transform({ x, y });
}

void GR::SampleDynamicObject::translate(const sf::Vector2f& vec) {
	translate(vec.x, vec.y);
}

void GR::SampleDynamicObject::putForceToCenter(float x, float y) {
	box2dModel->putForceToCenter({x, y});
}

void GR::SampleDynamicObject::putForceToCenter(const sf::Vector2f& vec) {
