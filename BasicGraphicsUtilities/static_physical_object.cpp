...
GR::StaticPhysicalObject::StaticPhysicalObject(b2World& world, const float& time, std::vector<std::pair<float, float>> vertices, std::string texture_path)
	...
	for (int i = 0; i < vertices.size(); i++) {
		...
		verts[vertices.size()- i - 1] = { vertices[i].first - x, -(vertices[i].second - y) };
	}
